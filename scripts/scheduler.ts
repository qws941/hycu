 ...
 * scheduler.ts (legacy)
 *
 * Long-running weekday scheduler. Kept for backward compatibility.
 * New deployments should use the HTTP server (src/server.ts) with
 * n8n or cron for scheduling.
 */

import { config } from '../src/config.js';
import { runAttendanceCycle } from '../src/attendance-runner.js';
...
async function runAttendance(): Promise<void> {
  ...
  if (result.success) {
    console.log(`[scheduler] ✅ ${result.message} (${result.attempts} attempt(s))`);
  } else {
    console.error(`[scheduler] ❌ ${result.message} (${result.attempts} attempt(s))`);
    console.error('[scheduler] Next scheduled run will retry.');
  }
}
...
console.log('[scheduler] HYCU attendance scheduler started (legacy mode)');
